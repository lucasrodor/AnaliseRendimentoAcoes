...
def main():
    ...
    carteira_roi = sum([r for _, r in rendimentos_roi if r is not None]) / len(rendimentos_roi) #RENDIMENTO DA CARTEIRA
    print(f"\nAções acima do Ibovespa:")
    for k in acima_ibov_roi:
        print(f'-{k}')
    print(f"\nAções abaixo do Ibovespa:")
    for l in abaixo_ibov_roi:
        print(f'-{l}')
    print(f"\nRendimento carteira ROI = {carteira_roi:.2f}%")
    print("---------------------------------------------------")
    print("Magic Formula:")
    for ticker, rendimento in rendimentos_mf:
        if rendimento is not None:
            print(f"Rendimento total da ação {ticker}: {rendimento:.2f}%")
    carteira_mf = sum([r for _, r in rendimentos_mf if r is not None]) / len(rendimentos_mf) #RENDIMENTO DA CARTEIRA
    print(f"\nAções acima do Ibovespa:")
